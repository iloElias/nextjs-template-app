...
- **React:** Upgrade to v19+
- **HeroUI:** Update all packages to v3
- **Tailwind CSS:** Update to v4
- **Framer Motion:** Remove (no longer required)

### Provider Changes

v3 removes the need for `HeroUIProvider`. You can delete it from your app root:
...
Many components in v3 use React Aria Components patterns, which involve splitting components into compound parts for better customization and accessibility.

### Hooks Removed

v2 component hooks like `useSwitch` or `useInput` are removed - use compound components instead. `useDisclosure` is replaced with `useOverlayState`. See the [Hooks Migration Guide](/docs/react/migration/hooks) for details.
